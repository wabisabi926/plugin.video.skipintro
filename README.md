...
### v1.3.7 (2026-05)
- ✅ **统一通知函数**：将 `show_notification` 函数统一到 `common.py`，消除代码重复
- ✅ **完善异常处理**：新增 `log_exception` 函数，提供统一的异常日志格式和完整堆栈跟踪
- ✅ **添加类型提示**：为关键函数和类添加 `typing` 类型注解，提高代码可读性和 IDE 支持
- ✅ **缓存清理机制**：新增定期缓存清理功能，限制缓存大小（最多 50 条），防止内存泄漏
- ✅ **配置常量集中化**：将魔法数字集中定义为常量，便于调整参数
- ✅ **代码质量提升**：全面优化代码结构，提升可维护性
