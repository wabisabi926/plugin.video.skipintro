...
- ✅ **安全写入**：原子写入 + 防抖机制，减少磁盘占用
- ✅ **播放列表补全**：解决 OSD 播放列表偶发性排序错乱问题（默认关闭）
- ✅ **删除所有记录**：支持一键清除全部跳过时间点
...
- 🐛 **修复记录失败**：修复部分情况下记录跳过点时返回失败的问题
- 🐛 **修复倒计时异常**：修复跳过倒计时偶发计时不准的问题
- 🐛 **修复删除报错**：修复删除记录时可能报错的问题
- ⚡ **性能优化**：跳过点保存更快，响应更及时
- ⚡ **内存优化**：长时间使用后内存占用更稳定
- 🧹 **简化使用**：移除不必要的「自动播放设置」弹窗提醒
- 🔧 **代码健壮**：减少后台异常，降低崩溃概率
